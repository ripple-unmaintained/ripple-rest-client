...
Client.prototype.getNextNotification = function(opts, fn){
  ...
  if (opts.previousTransactionHash) {
    url += ('/'+opts.previousTransactionHash);
  }
...
Client.prototype.getPayment = function(opts, fn){
  var url = this.api+'api/v1/addresses/'+this.account+'/payments/'+opts.transactionHash;
...
Client.prototype.getTransaction = function(opts, fn){
  var url = this.api+'api/v1/addresses/'+this.account+'/txs/'+opts.transactionHash;
  request.get(url, {form: opts, json: true }, function(err, resp, body) {
    fn(err, body);
  }) 
};
...
Client.prototype.getServerStatus = function(opts, fn){
  var url = this.api+'api/v1/status';
  request.get(url, {form: opts, json: true }, function(err, resp, body) {
    fn(err, body);
  }) 
};
